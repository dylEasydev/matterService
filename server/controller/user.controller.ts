...
export class UserController extends BaseController{
    async findAllMatter(req:Request, res:Response){
        if(req.params.id){
            try {
                const userId = isNaN(parseInt(req.params.id))?0:parseInt(req.params.id);
                ...
                if(req.query.searh){
                    ...
                    const tableMatter = await matterService.findAllMatterTeacher(userId,limit , search);
                    return statusResponse.sendResponseJson(
                ...
                const tableMatter = await matterService.findAllMatterTeacher(userId,limit);
                return statusResponse.sendResponseJson(
                    CodeStatut.VALID_STATUS,
                    res,
